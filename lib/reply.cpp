...
namespace http10
{
    namespace server
    {
        ...
        std::string reply::to_buffers()
        {
            std::stringstream buffers;
            buffers << (status_strings::to_buffer(status));
            for (std::size_t i = 0; i < headers.size(); ++i)
            {
                header& h = headers[i];
                buffers << (h.name + ":" + h.value);
                buffers << (std::string(misc_strings::crlf));
            }
            buffers << (std::string(misc_strings::crlf));
            buffers << (std::string(content));
            return buffers.str();
        }
        ...
        reply reply::html_reply(reply::status_type status)
        {
            reply rep;
            rep.status = status;
